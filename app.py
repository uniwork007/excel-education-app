...
def analyze_excel_file(file_stream, stage_id):
  ...
  # stage7も2シート構成。「データ」以外のシートを採点対象とする
  if stage_id == 4 and len(wb.sheetnames) > 1:
    ws = next((wb[s] for s in wb.sheetnames if s != "マスタ"), wb.active)
  elif stage_id == 7 and len(wb.sheetnames) > 1:
    ws = next((wb[s] for s in wb.sheetnames if s != "データ"), wb.active)
  else:
    ws = wb.active
  detected_errors = []
  ...
  if stage_id == 6:
    ...
    # グラフ種類のチェック（棒グラフ・折れ線グラフが両方あるか）
    if charts:
      from openpyxl.chart import BarChart, LineChart
      chart_types = [type(c).__name__ for c in charts]
      has_bar  = any(isinstance(c, BarChart)  for c in charts)
      has_line = any(isinstance(c, LineChart) for c in charts)
      detected_errors.append(
          f"【参考情報】検出されたグラフ種類: {', '.join(set(chart_types))}")
      if not has_bar:
        detected_errors.append(
            "⚠️棒グラフ（集合縦棒）が検出されませんでした。"
            "売上・目標を棒グラフで表示しましょう。")
      if not has_line:
        detected_errors.append(
            "⚠️折れ線グラフが検出されませんでした。"
            "達成率を折れ線グラフに変更して複合グラフにしましょう。")
